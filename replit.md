...
## Monorepo Structure
The project is organized as a clean monorepo with the following structure:
- **backend/** - Express.js API server (previously `server/`)
- **frontend-web/** - React + Vite web application (previously `client/`)
- **mobile-app/** - Expo React Native mobile client (previously `odanet-mobile/`)
- **packages/odanet-api/** - Shared API SDK for all applications
- **shared/** - Shared TypeScript types and utilities
- **.env.shared** - Shared environment variables across all apps
...
- **Frontend (Web)**: React with TypeScript (Vite) located in `frontend-web/`, Tailwind CSS, shadcn/ui, and Radix UI. State management uses React Query and React hooks. Wouter provides client-side routing, and i18next handles internationalization (Turkish primary locale). Detail pages use SEO-friendly slug-based routing.
- **Frontend (Mobile)**: Built with Expo SDK and React Native in `mobile-app/`, utilizing Expo Router for file-based navigation. Styling is managed via React Native's StyleSheet API with a centralized theme system. TanStack Query handles data fetching, and SecureStore manages JWT tokens. The home screen displays a unified chronological feed of listings and seekers. Login/Register and Profile screens feature gradient headers and themed components. Bottom tab navigation uses purple active states.
- **Backend**: Developed with Express.js and TypeScript in `backend/`, using PostgreSQL with Drizzle ORM. The API is RESTful, providing detail endpoints with eager-loaded relations and enforcing publish/active filters.
- **Shared API SDK**: Located in `packages/odanet-api/`, provides a unified API client for all applications (web, mobile, backend scripts) with TypeScript support and Axios-based HTTP client.
...
- **Turkish Location System**: Integrated `turkey-neighbourhoods` NPM package provides comprehensive Turkish location data (81 cities, 973 districts, 50,000+ neighborhoods). The CreateListing and CreateSeekerProfile forms use cascading dropdowns for city/district/neighborhood selection, ensuring data consistency and user-friendly location input. Both listings and seeker profiles store structured location fields (city, citySlug, district, districtSlug, neighborhood, neighborhoodSlug) for precise filtering and search. The backend rebuilds the legacy `preferredLocation` field from structured data for backward compatibility. Schema validation is split into two schemas for seeker profiles: `insertSeekerProfileSchema` (with location refinement for creation) and `updateSeekerProfileSchema` (without refinement for partial updates), ensuring create-time validation while allowing targeted edits like publish toggles.
