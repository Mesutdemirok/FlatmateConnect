...
- **Frontend (Mobile)**: Expo SDK 54 + React Native 0.81.5, Expo Router for file-based navigation, NativeWind v4 for Tailwind styling with custom brand colors (primary: #00A6A6, accent: #007878, background: #F8F8F8), TanStack Query for data fetching, SecureStore for JWT token management. Custom UI components (PrimaryButton with gradients, SecondaryButton, SearchInput, ListingCard) built with Tailwind className syntax. Bottom tab navigation with Ionicons (Home, Messages, Favorites, Profile). Production-ready with EAS build configuration for Android and iOS deployment.
