...
### UI/UX Decisions
The platform features a mobile-first design with consistent branding utilizing teal/turquoise primary color (#00A6A6). Web app uses shadcn/ui with Radix components. Mobile app features custom Tailwind components with gradient buttons, clean white cards with shadows, rounded corners (2xl), and icon-based navigation. Logo displays at top of home screen with platform tagline. Search input with icon, two action buttons (Oda arıyorum/Oda veriyorum), followed by listing feed. All screens use real API data with proper loading and empty states.
...
- **Frontend (Mobile)**: Expo SDK 54 + React Native 0.81.5, Expo Router for file-based navigation, React Native StyleSheet API for all styling (no React Native Paper dependencies), custom brand colors (primary: #00A6A6, accent: #007878, background: #FAFAFA), TanStack Query for data fetching, SecureStore for JWT token management. Custom UI components (PrimaryButton with gradient styling, SecondaryButton with outlined style, SearchInput, ListingCard with image error handling) built with StyleSheet. Home screen shows first 5 listings with "Tüm İlanları Gör" button navigating to full listings page. Images use onError fallback to display teal placeholder (🏠 emoji + "Fotoğraf Yok") when CDN images fail or are corrupt. Bottom tab navigation with Ionicons (Home, Messages, Favorites, Profile). Production-ready with EAS build configuration for Android and iOS deployment.
